...
- 专属训练计划，支持独立草稿/发布按钮、历史归档及删除恢复，数据库阻止学员访问他人计划。
- 学员专属单次/不限次数包月价格，数据库按学员隔离；Resend 专属 Segment 同步及失败重试。
...
本地数据库测试不能替代真实 Supabase Auth、PostgREST、Resend、Cron 和 Vercel 的联调。上线前应完成 ACCEPTANCE.md 中的真实双学员验收。Stripe 已实现一次性购买、订单及付款后入账，目前配置为教练专用测试模式；包月到期手动购买，不自动续费。退款会记录并提示教练核对权益。配置和验证状态见 STRIPE_SETUP.md。
